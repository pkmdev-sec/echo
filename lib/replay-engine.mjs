...
export class TimelineEntry {
  constructor(event, relativeMs) {
    ...
    if (!event || typeof event !== 'object') {
      throw new Error('TimelineEntry requires a valid event object');
    }
    if (!event.type || typeof event.type !== 'string') {
      throw new Error('TimelineEntry event must have a valid type');
    }
    if (typeof event.timestamp !== 'number' || isNaN(event.timestamp)) {
      throw new Error('TimelineEntry event must have a valid timestamp');
    }
    if (typeof relativeMs !== 'number' || isNaN(relativeMs)) {
      throw new Error('TimelineEntry relativeMs must be a valid number');
    }
    this.event = event;
...
export class ReplayEngine {
  constructor(sessionData) {
    ...
    this._timeline = null;
    this._cursor = 0;
    this._playbackSpeed = 1.0; // 1x default
    this._eventFilter = null; // null means show all
    this._bookmarks = [];
  }
  ...
  setPlaybackSpeed(speed) {
    const validSpeeds = [0.5, 1.0, 2.0, 4.0];
    if (!validSpeeds.includes(speed)) {
      throw new Error(`Invalid playback speed. Must be one of: ${validSpeeds.join(', ')}`);
    }
  ...
  setEventFilter(filter) {
    // filter can be null (show all), a string (event type), or an array of event types
    if (filter === null || typeof filter === 'string' || Array.isArray(filter)) {
      this._eventFilter = filter;
    } else {
      throw new Error('Event filter must be null, a string, or an array of strings');
    }
  }
  ...
  addBookmark(name, index = null) {
    const bookmarkIndex = index !== null ? index : this._cursor;
    if (bookmarkIndex < 0 || bookmarkIndex >= this.events.length) {
      throw new RangeError(`Bookmark index ${bookmarkIndex} out of bounds`);
    }
    const bookmark = {
      name,
      index: bookmarkIndex,
      event: this.events[bookmarkIndex],
      timestamp: Date.now(),
    };
  ...
  seekToBookmark(name) {
    const bookmark = this._bookmarks.find(b => b.name === name);
    if (!bookmark) {
      throw new Error(`Bookmark not found: ${name}`);
    }
    return this.seek(bookmark.index);
  }
  ...
  getFilteredTimeline() {
    ...
    if (!this._eventFilter) return timeline;

    if (typeof this._eventFilter === 'string') {
      return timeline.filter(entry => entry.event.type === this._eventFilter);
    }
    if (Array.isArray(this._eventFilter)) {
      return timeline.filter(entry => this._eventFilter.includes(entry.event.type));
    }
    return timeline;
  }
  ...
  skipToNextToolCall() {
    const timeline = this.getTimeline();
    for (let i = this._cursor + 1; i < timeline.length; i++) {
      if (timeline[i].event.type === 'tool_use') {
        this._cursor = i;
        return timeline[i];
      }
    }
  ...
  skipToPrevToolCall() {
    ...
    return null; // No previous tool calls
  }

  skipToNextError() {
    const timeline = this.getTimeline();
    for (let i = this._cursor + 1; i < timeline.length; i++) {
      if (timeline[i].event.type === 'error') {
        this._cursor = i;
        return timeline[i];
      }
    }
